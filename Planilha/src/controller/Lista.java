...
public class Lista<T> implements java.lang.Iterable<T> {
	...
	public void remove(Celula<T> cel) { // não está funcionando
		
		atual = cel;		
		if (atual == null) {
			throw new IllegalStateException("Atual é null");
		} else {
			Celula<T> anterior = atual.getPrevious();
			Celula<T> atual = anterior.getNext();
			Celula<T> proximo = atual.getNext();
